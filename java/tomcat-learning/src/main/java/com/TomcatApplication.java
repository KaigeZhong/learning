...
public class TomcatApplication {
    ...
    public static void main(String[] args) throws LifecycleException {
        ...
        /*
        设置catalinaHome和catalinaBase，也可以通过tomcat.setBaseDir(Thread.currentThread().getContextClassLoader().getResource("").getFile())进行设置
        ...
        */
        server.setCatalinaHome(new File(Thread.currentThread().getContextClassLoader().getResource("").getFile()));
        server.setCatalinaBase(new File(Thread.currentThread().getContextClassLoader().getResource("").getFile()));
         ...
         * 配置servlet
         */
        //设置servlet,也可以调用tomcat.addServlet(context, "homeServlet", new HomeServlet());
        tomcat.addServlet("/book", "homeServlet", new HomeServlet());

        //添加servlet的映射，curl http://localhost:9080/book/hello
